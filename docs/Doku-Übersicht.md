...
|---|---|---|---|
| `Produkt-Anforderung.md` | Fachliches Produktbild von Leif OS: Zielbild, Rolle des Systems, Hauptaufgaben, Maintabs, Kernobjekte, Statusmodelle, Seitenarten, Grundprinzipien. | *Was ist Leif OS fachlich und welche Bausteine gehören konzeptionell dazu?* | V1-Abschneidung, detaillierte Seitenaufteilung, konkrete Routen/Dateipfade, Tabellen-/Feldlisten. |
| `V1-Scope.md` | Bewusster Erstumfang für den Neustart: Welche Teile des Produktbilds gehören in V1, welche bewusst nicht. | *Was bauen wir jetzt wirklich – und was lassen wir in dieser Runde weg?* | Endgültiges Zielbild (gehört in Produkt-Anforderung), technische Umsetzung, detaillierte Seite-zu-Route- oder Tabellen-Details. |
| `Route-Map.md` | Fachliche Seiten- und Zuständigkeitskarte: globaler App-Rahmen, alle Seiten/Seitentypen mit Zweck, sichtbaren Objekten, Hauptaktionen und Abgrenzung zueinander. | *Wo in der Oberfläche lebt welche Funktion, und wofür ist jede Seite zuständig?* | Produktvision/Philosophie, V1/Später-Entscheidungen, technische Routen, Komponenten, Tabellen. Das gehört in Produkt-Anforderung, V1-Scope oder Implementation-/Datenmodell-Doku. |
...
| `Implementation-Map.md` | Technische Verortung der Funktionen: pro Funktion Seite, Serveraktionen/Queries, beteiligte Tabellen und Workflows. Kurze **Repo-Anker** (z. B. Server-Action-Name, einzelner Modulpfad) sind ergänzend erlaubt; keine Code-Templates und keine Komponenten-Vollkataloge. | *Welche Funktion wird wo gebaut, was braucht sie serverseitig, und welche Tabellen und Abläufe greifen?* | Produktziele, fachliche Scope-Fragen, rein fachliche Seitenbeschreibung ohne technische Konsequenz, Roh-Schema des Datenmodells. |
...
| `Erledigte-Aufgaben.md` | Kumulativer Überblick über **bereits umgesetzte** Entwicklungs- und Doku-Schritte (Repo-Stand). | *Was ist schon gebaut, ohne die fachliche Spezifikation zu ersetzen?* | V1-Zielbild, verbindliche Anforderungen, detaillierte technische Verortung (→ V1-Scope, Produkt-Anforderung, Implementation-Map). |
...
3. **Konsistenz zwischen Dateien**
   Wo Themen in mehreren Dateien auftauchen (z.B. Dashboard in Produkt-Anforderung, V1-Scope und Route-Map), müssen Aussagen konsistent bleiben und sich nur in der Flughöhe unterscheiden (Endbild vs. V1 vs. Seiten-Zuständigkeit).
...
5. **Keine technische Details in Produkt-/Route-Map-Dokus**  
   Dateipfade, Komponentenbäume, konkrete Actions/Queries oder Tabellen gehören in `Implementation-Map.md` und `Datenmodell.md`, nicht in `Produkt-Anforderung.md`, `V1-Scope.md` oder `Route-Map.md`.
...
| 2026-04-11 | Implementation-Map: Rolle der Datei um **Repo-Anker** ergänzt (kein reiner „Stub ohne Pfade" mehr) | Abgleich mit umgesetztem Code; nachvollziehbare Verweise ohne Vollinventar. |
| 2026-04-11 | `Erledigte-Aufgaben.md` neu; in der Doku-Typen-Tabelle verlinkt | Transparenz des Umsetzungsstands neben der fachlichen Spezifikation. |
| 2026-04-12 | Tabellenzeile Produkt-Doku: Dateiname auf `Produkt-Anforderung.md` korrigiert; Arbeitsregel 5 angepasst | Einheitlicher Dateiname im Repo; Tippfehler „Anforderungen" vermeiden. |
